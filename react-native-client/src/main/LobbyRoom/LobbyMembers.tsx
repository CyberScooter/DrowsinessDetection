...
export default function LobbyMembers({ route }) {
  ...
  async function loadMembers() {
    ...
    axios
      .get(`${restAPIURL}/api/lobby/members?lobbyID=${route.params?.lobbyID}`, {
        headers: {
    ...
    flatList = (
      <View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            if (item.id == driverID) {
              currentlyDrivingComponent = <Text>Currently driving</Text>;
            } else {
                ...
                <View style={styles.item}>
                  ...
                  {ownerID != item.id ? (
                    <Button
                      color="darkred"
                      title="Remove Member"
                      onPress={() => removeUser(route.params?.lobbyID, item.id)}
                    />
                  ) : null}
